from model2d.stacked_unet2d import StackedUnet2D
...
def run():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (image, label, index) in enumerate(train_dataset):
            ...
            optimizer.zero_grad()
            predicts = net(image) # 5D tensor   Batch_Size * 4 * 2 * H * W

            # ****** calculate loss ******
            loss_train = 0
            for k in range(4):
                loss_train += criterion(predicts[:, k, :, :, :], label[:, 0, :, :].long())
        ...
        for step, (image, label, subject) in enumerate(test_dataset):
            ...
            loss_test = 0
            for k in range(4):
                loss_test += criterion(predicts[:, k, :, :, :], label[:, 0, :, :].long())
